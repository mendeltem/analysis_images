...
DEFAULT_IMAGE_DIRECTORY     = config["MEMORY_IMAGES"]
...
def get_feature(p, fovea, image, selected_data): 
    ...
    for layer in range(len(p)):
        ...
        temp_df.columns = [str(layer+1) + "_" + str(col+1) for col in temp_df.columns]
...
def sel_experiment_data(all_data, pic_id, color, masktype, maskregion ):
...
for i,image in enumerate(img_sequ): 
    ...
    save_dir_temp = SAVE_DIR +"/"+model_type+"/memory"
...
#    
    ...
    if "original" in img_paths[i]:
        ...
        feather.write_dataframe(
            df,save_dir_temp+"/original/" +str(pic_id)+".file")
        ...
        feather.write_dataframe(
            df,save_dir_temp+"/zen_hp/original/" +str(pic_id)+".file")
        ...
        feather.write_dataframe(
            df,save_dir_temp+"/zen_lp/original/" +str(pic_id)+".file")
        ...
        feather.write_dataframe(
            df,save_dir_temp+"/per_hp/original/" +str(pic_id)+".file")
        ...
        feather.write_dataframe(
            df,save_dir_temp+"/per_lp/original/" +str(pic_id)+".file")
        ...
        create_dir(save_dir_temp+"/zen_hp/filterd/" )
        
        feather.write_dataframe(
            df,save_dir_temp+"/zen_hp/filterd/" +str(pic_id)+".file")
        ...
        create_dir(save_dir_temp+"/per_hp/filterd/" )
        
        feather.write_dataframe(
            df,save_dir_temp+"/per_hp/filterd/" +str(pic_id)+".file")
        ...
        create_dir(save_dir_temp+"/zen_lp/filterd/" )
        
        feather.write_dataframe(
            df,save_dir_temp+"/zen_lp/filterd/" +str(pic_id)+".file")
        ...
        create_dir(save_dir_temp+"/per_lp/filterd/" )
        
        feather.write_dataframe(
            df,save_dir_temp+"/per_lp/filterd/" +str(pic_id)+".file")
...
##        
#        Output_df.to_csv("/mnt/data/DATA/dataset/FEATURES/saved_features_from_pictures/exception/Output_per_"+str(saved)+".h5")
...
#        
#def combine_feature_withexperiment_data():
#
#
#
#
...
#type_feature = "greyscale_original"
#
#saved_features  = '/mnt/data/DATA/dataset/FEATURES/saved/'+type_feature+'/h5'
#
#paths = get_data_paths(saved_features)
#
#indezes = []
#
#cols = feature_1.columns
#
...
#for path in paths:
#  print(path)
#  feature_1 = pd.read_pickle(path)
#  
#  if len(feature_1) != 0:
#    
#    pic_id = int(feature_1.loc[0,'imageid'])
#    
#  
#    mean_list = [] 
#    
...
#        mean = feature_1.iloc[:, indezes[index]:-1].mean(axis=(1)).mean()  
#      else:
#        mean = feature_1.iloc[:, indezes[index]:indezes[index+1]].mean(axis=(1)).mean()  
#         
#      mean_list.append(mean)  
#      print(mean)
#    
#    row = np.concatenate([[pic_id],mean_list])
#    
#    Mean_df = pd.concat([Mean_df, pd.Series(row).transpose()  ], axis=1)
#
#Mean_df = Mean_df.transpose()
#
#Mean_df.to_csv( type_feature+"_mean.csv")
#
#
